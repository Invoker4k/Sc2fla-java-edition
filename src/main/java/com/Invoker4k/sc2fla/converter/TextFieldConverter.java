...
public class TextFieldConverter {
    ...
    public TextFieldConverter(Document doc, Map<Integer, String> directExportNameById, ConverterConfig config) {
        this.doc = doc;
        this.directExportNameById = directExportNameById;
        this.config = config;
    }

    public void convert(TextFieldOriginal tf) throws Exception {
        ...
        dynamicText.lineType = tf.isMultiline() ? "multiline no wrap" : "single line";
        dynamicText.fontRenderingMode = tf.isUseDeviceFont() ? "device" : "standard";
        if (tf.isAutoAdjustFontSize()) dynamicText.autoExpand = true;
        ...
        if (tf.isOutlineEnabled() && tf.getOutlineColor() != 0) {
            TextField.GlowFilter glow = new TextField.GlowFilter();
            glow.color = tf.getOutlineColor() & 0x00FFFFFF;
            glow.strength = 15;
            glow.blurX = 2;
            glow.blurY = 2;
        ...
        String directExportName = directExportNameById.get(tf.getId());
